...
int main(){
    ...
    agent agent("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR",'b',6);
    ...
    while(window.isOpen()){
        if (chess.whiteTurn()){ 
            ...
            agent.UploadBoard(chess.getBoard());
            
            agent.GetBestMove();
            chess.selectPiece(chess.mapping[agent.from]);
            chess.moveSelected(chess.mapping[agent.to]); 
        }
